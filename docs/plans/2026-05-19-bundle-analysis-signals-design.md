...
- Tool-to-bundle: inventory-declared tools plus a maintained set for known bundle-contributed tools (`graph_query`, `blob_read`, `dot_graph`, `team_knowledge`, `terminal_inspector`, `generate_image`) and an `mcp_*` prefix convention (→ `deepwiki` bundle).
...
- **Tool-to-bundle mapping maintenance.** The bundle-contributed tool set (`graph_query`, `blob_read`, `dot_graph`, `team_knowledge`, `terminal_inspector`, `generate_image`) is maintained alongside the `mcp_*` prefix convention. As inventory grows, the hardcoded set may lag. Future work: have inventory itself declare the tools each bundle contributes so the maintained set can be retired.
