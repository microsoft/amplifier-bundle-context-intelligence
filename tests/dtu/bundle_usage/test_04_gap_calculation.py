...
def test_gap_arithmetic_consistent(dtu_session):
    ...
    for bundle_name, pb in per_bundle.items():
        aa = pb.get("always_active", {})
        util_pct = aa.get("util_pct", {})
        for k in ("agents", "skills", "modes", "recipes"):
            util_val = util_pct.get(k)
            assert util_val is None or 0.0 <= util_val <= 1.0, (
                f"Arithmetic inconsistency: util_pct[{k!r}] out of range for bundle {bundle_name!r}. "
                f"util_pct={util_pct}, "
                f"always_active={aa}"
            )
...
def test_foundation_util_gap_present(dtu_session):
    ...
    aa = pb["always_active"]
    declared_agents = aa["declared"]["agents"]  # list[str] (serialised set)
    used_agents = aa["used"]["agents"]  # list[str] (serialised set)

    assert len(declared_agents) > 1, (
        "Expected gap.per_bundle['foundation']['always_active']['declared']['agents'] to have > 1 element "
        f"but got {len(declared_agents)}: {declared_agents}. "
    ...
    assert len(used_agents) >= 1, (
        "Expected gap.per_bundle['foundation']['always_active']['used']['agents'] to have >= 1 element "
        f"but got {len(used_agents)}: {used_agents}. "
        "The ground-truth session invokes multiple foundation agents "
        "(explorer, git-ops, etc.); at least one must appear in the signals."
    )

    util_pct = aa["util_pct"]["agents"]
    assert util_pct is None or 0.0 <= util_pct <= 1.0, (
        f"Expected gap.per_bundle['foundation']['always_active']['util_pct']['agents'] "
        f"to be None or in [0.0, 1.0] but got {util_pct!r}."
    )
...
def test_improvement_entries_well_formed(dtu_session):
    ...
    # Per-entry validation for tree-shake: must have names (list), component_type, and scope.
    tree_shakes = [i for i in imp if i["type"] == "tree-shake"]
    for ts in tree_shakes:
        assert "names" in ts and isinstance(ts["names"], list), (
            f"tree-shake entry missing 'names' list: {ts!r}"
        )
        assert "component_type" in ts and ts["component_type"] in (
            "agents",
            "skills",
            "recipes",
            "context",
            "tools",
            "modes",
        ), f"tree-shake entry has invalid or missing 'component_type': {ts!r}"
        assert ts["scope"] == "always_active", (
            f"tree-shake entry expected scope='always_active' but got {ts.get('scope')!r}: {ts!r}"
        )
