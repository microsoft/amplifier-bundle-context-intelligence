...
check("D1  Facilitator explicitly declares tool-delegate in tools:",
      "tool-delegate" in fac_tools,
      f"declared tools: {list(fac_tools.keys())}")
check("D2  Tool-designer explicitly declares tool-delegate in tools:",
      "tool-delegate" in des_tools,
      f"declared tools: {list(des_tools.keys())}")

# D3: Mode has 'delegate' in safe list
check("D3  Mode tools.safe contains 'delegate' (root-session policy)",
      "delegate" in mode_safe)
...
check("D4  Facilitator frontmatter has NO mode.tools.safe block (not relying on mode policy)",
      not fac_safe,
      f"fac mode.tools.safe={fac_safe}")
check("D4b Tool-designer frontmatter has NO mode.tools.safe block",
      not des_safe,
      f"des mode.tools.safe={des_safe}")

# D5: Both declare tool-skills (not inherited from parent session)
check("D5  Facilitator explicitly declares tool-skills in tools:",
      "tool-skills" in fac_tools,
      f"declared tools: {list(fac_tools.keys())}")
check("D5b Tool-designer explicitly declares tool-skills in tools:",
...
check("D6a Facilitator's tool-delegate source is amplifier-foundation",
      foundation_domain in fac_delegate_src,
      f"source={fac_delegate_src}")
check("D6b Tool-designer's tool-delegate source is amplifier-foundation",
...
check("D7a Facilitator's tool-skills source is amplifier-bundle-skills",
      skills_domain in fac_skills_src,
      f"source={fac_skills_src}")
check("D7b Tool-designer's tool-skills source is amplifier-bundle-skills",
...
check("D8a Facilitator's tool-skills config points to CI bundle skills",
      any(ci_bundle in s for s in fac_skills_cfg),
      f"config.skills={fac_skills_cfg}")
check("D8b Tool-designer's tool-skills config points to CI bundle skills",
...
class FakeCoordinator:
    ...
    def get(self, key): return {t.name: t for t in self.tools}.get(key)
...
check("D9  Sub-session WITH tool-delegate: can call delegate()",
      can_delegate_with)
check("D9b Sub-session WITHOUT tool-delegate: CANNOT call delegate() — proves explicit declaration is required",
...
if failures:
    ...
    print("Both agents satisfy Brian's operational note:\n"
