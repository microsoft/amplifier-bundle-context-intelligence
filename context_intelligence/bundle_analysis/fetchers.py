...
Normalizes inputs from two sources (graph rows and JSONL records) into a common
RawSignalEvent dataclass with four discriminated kinds:
...
GraphFetcher executes pre-written Cypher files (session_signals.cypher or
workspace_signals.cypher) via AsyncCIClient and normalizes each row.
...
SignalKind = Literal["agent_spawned", "skill_loaded", "recipe_execute", "mentions_resolved"]
...
class GraphFetcher:
    """Fetches raw attribution signals from the context-intelligence graph.

    Selects the query file based on *session_id*:

    * ``session_signals.cypher`` — when *session_id* is not ``None``
    * ``workspace_signals.cypher`` — when *session_id* is ``None``

    The result is a tuple ``(events, server_ok)`` where ``server_ok`` is
    ``True`` when the server responded (even with an empty result set) and
    ``False`` when the client raised an exception.
    """
    ...
    async def fetch(
        ...
        *,
        ...
        workspace: str,
        session_id: str | None,
    ) -> tuple[list[RawSignalEvent], bool]:
        """Execute the appropriate Cypher query and return normalised events.
        ...
        client:
            Async CI client.
        workspace:
            Workspace to scope the query.
        session_id:
            Optional session ID; when provided the session-scoped query is used.

        Returns
        -------
        tuple[list[RawSignalEvent], bool]
            ``(events, server_ok)`` — ``server_ok`` is ``False`` when the
            client raised an exception, ``True`` otherwise (including empty).
        """
        if session_id is not None:
            query_file = _queries_dir() / "session_signals.cypher"
            params: dict[str, Any] = {"session_id": session_id}
        else:
            query_file = _queries_dir() / "workspace_signals.cypher"
        ...
        try:
            ...
            logger.warning("GraphFetcher: cypher query failed: %s", exc)
            return ([], False)
...
__all__ = ["RawSignalEvent", "GraphFetcher", "JSONLFetcher"]
