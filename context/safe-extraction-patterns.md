...
```bash
cd ~/.amplifier/projects/{slug}/sessions/{session_id}
```
...
```bash
jq -c 'select(.event == "orchestrator:error" or .event == "tool:error") | {event: .event, ts: .timestamp, error: .data.error}' events.jsonl
```
