...
Three composable behaviors for session observability + context intelligence: a telemetry hook
(logging), analytics tools + agents (analytics), and a full behavior combining both. Ships as a
bundle with behaviors in `behaviors/`.
...
| Path | Contents |
|------|----------|
| `behaviors/` | Three behavior YAMLs: `context-intelligence-logging`, `-analytics`, and the full `context-intelligence` |
...
|----------|----------------|
| **S1** | Analytics-only sync — skill fetched and discoverable after `on_session_ready` |
...
| **S4** | Full behavior — telemetry hook + analytics sync both active |
...
- **Analytics-mode config placeholders** — `${AMPLIFIER_CONTEXT_INTELLIGENCE_*}` placeholders in
  behavior config are expanded by `ToolConfigResolver._expand`. In analytics-only mode (no hook),
