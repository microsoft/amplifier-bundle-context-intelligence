...
from .bundled_skill import EXPECTED_BUNDLED_SKILL_SHA256
...
_BUNDLED_SKILL_PACKAGE: str = "amplifier_module_tool_context_intelligence_query.bundled_skill"
...
def _install_vendored_body(skill_name: str, skill_path: Path) -> None:
    ...
    # --- SHA-256 integrity gate (fail loud, write nothing on mismatch) ----------
    # A tampered or corrupted wheel body must never reach disk silently.
    # EXPECTED_BUNDLED_SKILL_SHA256 is the authoritative pin burned into the package
    # at build time.  Any deviation means the wheel is broken or compromised.
    new_hash = _sha256_text(body)
    if new_hash != EXPECTED_BUNDLED_SKILL_SHA256:
        raise ValueError(
            f"skill_install_sha_mismatch: {skill_name!r} — "
            f"vendored body SHA {new_hash[:8]}… does not match expected pin "
            f"{EXPECTED_BUNDLED_SKILL_SHA256[:8]}…; refusing to install a "
            f"corrupted or tampered body. "
            f"(got={new_hash} expected={EXPECTED_BUNDLED_SKILL_SHA256})"
        )
    # ---------------------------------------------------------------------------
