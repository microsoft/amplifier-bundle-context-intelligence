"""Tests for SkillFetcher (relocated into tool-graph-query) — conditional HTTP GET."""
...
class TestVersionCapability:
    def test_is_skills_capable_none_returns_false(self) -> None:
        from amplifier_module_tool_graph_query.skill_fetcher import _is_skills_capable
...
class TestCheckServerVersion:
    async def test_connect_error_returns_unreachable(self) -> None:
        from amplifier_module_tool_graph_query.skill_fetcher import (
            SkillFetcher,
            VersionCheckResult,
        )
