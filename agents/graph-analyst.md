...
If environment variables are not set, find `server.url` and
`server.api_key` in the Amplifier bundle config YAML under
...
<!-- ConfigResolver fallback chain: how server.url, workspace, and log_level are resolved from env vars and settings -->
